...
$koneksi = mysqli_connect('localhost','swastami_root','t.d6!kEI0{Qf','swastami_db_swastamita');
	if (isset($_POST['simpanBarang'])) {
	    ...
	    $tglNow=date('dmy');
	    $hitung=mysqli_query($koneksi,"SELECT * FROM tb_barang WHERE id_kategori='$kategori' AND SUBSTR(id_barang, 4,6)='$tglNow'");
	    ...
	    $detailBarang=$_POST['detailBarang'];
	    ...
	    if ($rasio==1) {
	    	if ($imageSize<=$maxSize) {
	    		if (in_array($imageTipe, $ekstensi_diperbolehkan) === true) {
	                $insert=mysqli_query($koneksi,"INSERT INTO `tb_barang` (`id_barang`, `nama_barang`, `harga_barang`, `harga_sewa`, `qty`, `img`, `id_kategori`, `detail`) VALUES ('$idBarang', '$namaBarang', '$hargaBarang', '$hargaSewa', '$qty', '$imageName', '$kategori', '$detailBarang')");
	                if ($insert) {
